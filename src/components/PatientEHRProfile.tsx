...
export default function PatientEHRProfile({
  patientId,
  doctor,
  onBack,
  initialAppointment,
}: PatientEHRProfileProps) {
  ...
  const [workspaceMode, setWorkspaceMode] = useState<
    "consultation" | "prescription" | "lab" | "ai"
  >("consultation");
  ...
  const [symptoms, setSymptoms] = useState(
    initialAppointment?.symptoms ||
      "Minor retrosternal discomfort during physical exertion.",
  );
  const [observations, setObservations] = useState(
    "S1 & S2 dual heart sounds present. No murmurs or gallops. Lungs clear to auscultation bilaterally.",
  );
  ...
  const [clinicalNotes, setClinicalNotes] = useState(
    "Patient presented with episodic chest tightness. ECG shows sinus rhythm, no ST segment elevation. Initiated CCB monotherapy & ordered outpatient lipid panel.",
  );
  const [treatmentRecommendations, setTreatmentRecommendations] = useState(
    "1. Low sodium, heart-healthy dietary plan.\n2. Light aerobic exercise as tolerated.",
  );
  const [lifestyleAdvice, setLifestyleAdvice] = useState(
    "Avoid strenuous weightlifting until stress ECG is evaluated. Hydrate adequately.",
  );
  const [followUpInstructions, setFollowUpInstructions] = useState(
    "Review in Cardiology Outpatient Clinic in 2 weeks with repeat lipid results.",
  );
  ...
  const [newLabTestName, setNewLabTestName] = useState(
    "Full Blood Count (FBC) & Lipid Profile",
  );
  const [newLabPriority, setNewLabPriority] = useState<
    "Routine" | "Urgent" | "Emergency"
  >("Routine");
  const [newLabNotes, setNewLabNotes] = useState(
    "Rule out acute cardiac ischemia & hyperlipidemia.",
  );

  // Auto-Save Mechanism & Sub-Tab State for Clinical Workspace
  const [consultSubTab, setConsultSubTab] = useState<
    "vitals" | "notes" | "plan" | "summary"
  >("vitals");
  ...
  useEffect(() => {
    try {
      ...
      if (savedDraft) {
        ...
        if (parsed.treatmentRecommendations)
          setTreatmentRecommendations(parsed.treatmentRecommendations);
        if (parsed.followUpInstructions)
            ...
            medicalHistory: [
              {
                id: "HIS-1",
                type: "condition",
                title: "Mild Seasonal Asthma",
                date: "2020",
                notes: "Managed with Albuterol inhaler as needed",
              },
              {
                id: "HIS-2",
                type: "surgery",
                title: "Appendectomy",
                date: "2018",
                notes: "Performed at General Hospital Abuja. No complications.",
              },
            ],
          });
        }
  ...
  const handleAddDiagnosis = () => {
    ...
    setDiagnoses([
      ...diagnoses,
      { name: newDiagName.trim(), category: newDiagCategory },
    ]);
    setNewDiagName("");
  };
  ...
  const handleSaveConsultation = async () => {
    try {
      ...
      if (res.success) {
        try {
          localStorage.removeItem(`ehr_consult_draft_${patientId}`);
        } catch (e) {}
        setSaveSuccessMessage(
          "Consultation signed and appended to permanent EHR record successfully!",
        );
      ...
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center space-y-3">
          ...
          <p className="text-sm font-bold text-slate-700">
            Loading Patient Electronic Health Record (EHR)...
          </p>
        </div>
      </div>
    );
  ...
  const timelineItems: Array<{
  ...
  }> = [
    {
      ...
      symptoms:
        "Retrosternal chest tightness during strenuous physical activity.",
      clinicalNotes:
        "Cardiovascular exam: Regular rhythm, normal S1/S2. ECG reveals sinus rhythm without ischemic ST elevation. Prescribed Amlodipine 5mg Daily & ordered High-Sensitivity Troponin I assay.",
    ...
    },
    {
      ...
      symptoms:
        "Bilateral tight pressure around forehead after late work hours.",
      clinicalNotes:
        "Neurological examination completely intact. Cranial nerves I-XII normal. Motor strength 5/5 in all extremities. Recommended sleep hygiene and stress management.",
      prescriptions: [
        {
          id: "RX-901",
          patientId,
    ...
    },
    {
      ...
      clinicalNotes:
        "Visual Acuity: Right Eye 20/25, Left Eye 20/20. Fundoscopy: Normal optic disc margins, macula clear. Prescribed anti-glare corrective lens.",
      prescriptions: [],
      labRequests: [],
  ...
  const filteredTimeline = timelineItems.filter((item) => {
    ...
    const matchesHospital =
      hospitalFilter === "ALL" || item.hospital === hospitalFilter;
    const matchesDept =
      departmentFilter === "ALL" || item.department.includes(departmentFilter);
  ...
  return (
    <div className="h-screen w-screen bg-slate-100 flex flex-col font-sans text-slate-800 overflow-hidden">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-[11px] text-slate-500 font-medium">
                National Health Record • Age {patient?.age} • Blood Group{" "}
      ...
      <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 py-6 flex-1 grid grid-cols-12 gap-6 overflow-hidden h-[calc(100vh-4rem)]">
            ...
            <div className="space-y-3 text-xs">
              ...
              <div>
                <label className="text-[11px] font-bold text-slate-500 uppercase block mb-1.5 flex items-center gap-1">
                  <AlertCircle className="w-3.5 h-3.5 text-red-500" /> Known
                  Allergies
                </label>
                <div className="flex flex-wrap gap-1.5">
                  {patient?.allergies && patient.allergies.length > 0 ? (
                    patient.allergies.map((alg, idx) => (
                      <span
                        key={idx}
                        className="bg-red-100 text-red-800 font-bold text-[10px] px-2 py-1 rounded-md border border-red-200"
                      >
                        ⚠️ {alg}
                      </span>
                    ))
              ...
              <div className="bg-emerald-50 border border-emerald-200 p-3 rounded-xl text-[11px] space-y-1">
                ...
                <p className="text-emerald-700 text-[10px] leading-snug">
                  Patient granted active medical record consent to{" "}
                  <strong>General Hospital Abuja</strong>.
                </p>
              </div>
            </div>
          </div>
        ...
        <main className="col-span-12 lg:col-span-9 space-y-5 h-full overflow-y-auto pr-1">
          ...
          <div className="bg-white border border-slate-200 rounded-2xl p-1.5 shadow-2xs flex overflow-x-auto text-xs font-bold gap-1">
            ...
            >
              <FileSpreadsheet className="w-4 h-4" /> Imaging & Diagnostic
            ...
            <div className="bg-slate-50/50 border border-slate-200 rounded-2xl p-4 sm:p-6 shadow-2xs space-y-6">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-500 font-medium mt-0.5">
                      ...
                      • {doctor.hospitalName || "General Hospital Abuja"} (
              ...
              {saveSuccessMessage && (
                <div className="bg-emerald-50 border border-emerald-200 text-emerald-900 p-4 rounded-2xl flex items-center justify-between text-xs font-bold animate-in fade-in duration-200">
                  ...
                  <button
                    ...
                    className="text-emerald-700 hover:text-emerald-950 p-1 rounded-lg hover:bg-emerald-100 cursor-pointer transition-colors"
                  >
                    <X className="w-4 h-4" />
                  </button>
                </div>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <span className="font-extrabold text-blue-950 text-sm">
                      Active Patient: {patient?.name} ({patient?.id})
                    </span>
                    <p className="text-blue-800 text-xs font-medium mt-0.5">
                      Age: <strong>{patient?.age} Yrs</strong> • Gender:{" "}
                      <strong>Male</strong> • Phone: {patient?.phone}
                    </p>
                  </div>
                </div>
                  ...
                  <div className="bg-blue-50/70 border border-blue-200/80 rounded-xl px-4 py-2.5 flex flex-wrap items-center justify-between gap-3 text-xs">
                    <div className="flex items-center gap-4 flex-wrap">
                      <div className="flex items-center gap-1.5 text-slate-700 font-semibold">
                        ...
                        <span>
                          BP:{" "}
                          <strong className="text-slate-900">{vitalBP}</strong>
                        </span>
                        <span>
                          • HR:{" "}
                    ...
                    <div className="space-y-5 animate-in fade-in duration-200">
                        ...
                        <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                          <div className="flex items-center gap-2">
                            ...
                            <div>
                              ...
                              <p className="text-[11px] text-slate-500 font-medium">
                                Recorded clinical measurements & vital
                                parameters
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                          ...
                          <div>
                            <h3 className="font-extrabold text-slate-900 text-sm">
                              Symptoms & Physical Examination Notes
                            </h3>
                            <p className="text-[11px] text-slate-500 font-medium">
                              Patient history of present illness & systemic
                              findings
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs">
                          <div className="space-y-1.5">
                            <label className="font-extrabold text-slate-800 block text-xs">
                              Chief Complaints & Present Illness History
                            </label>
                            <textarea
                              rows={4}
                              value={symptoms}
                      ...
                      <div className="flex justify-end pt-1">
                        ...
                        >
                          <span>
                            Proceed to Step 2: Clinical Progress Notes
                          </span>
                      ...
                      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-4">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-extrabold text-slate-900 text-sm sm:text-base">
                                Permanent EHR Clinical Progress Notes
                              </h3>
                              <p className="text-[11px] text-slate-500 font-medium">
                                All changes automatically save to continuous
                                draft storage
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2 bg-slate-50 p-3 rounded-xl border border-slate-200/80 text-xs">
                          ...
                          <div className="flex flex-wrap gap-2">
                            ...
                            >
                              <Plus className="w-3 h-3 text-blue-500" /> SOAPE
                              Note
                            </button>
                            <button
                              type="button"
                      ...
                      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-slate-500 font-medium">
                              Categorized clinical conditions & diagnostic tags
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="space-y-4 text-xs">
                          <div className="flex flex-col sm:flex-row gap-2">
                            <select
                              ...
                              onChange={(e) =>
                                setNewDiagCategory(e.target.value)
                              }
                          ...
                          <div className="flex flex-wrap gap-2 pt-2 border-t border-slate-100">
                            {diagnoses.length === 0 ? (
                              <p className="text-xs text-slate-400 italic">
                                No working diagnoses added yet.
                              </p>
                            ) : (
                              diagnoses.map((diag, idx) => (
                                <span
                                  key={idx}
                                  className="bg-blue-50 border border-blue-200 text-blue-950 font-bold px-3.5 py-2 rounded-xl text-xs flex items-center gap-2 shadow-2xs"
                                >
                                  <span className="text-[10px] text-blue-600 font-extrabold uppercase bg-blue-100 px-2 py-0.5 rounded-md">
                                    {diag.category}
                                  </span>
                                  {diag.name}
                                  <button
                                    onClick={() => handleRemoveDiagnosis(idx)}
                                    className="text-blue-400 hover:text-red-500 font-bold ml-1 cursor-pointer transition-colors"
                                  >
                                    <X className="w-4 h-4" />
                                  </button>
                                </span>
                      ...
                      <div className="bg-white border border-slate-200/90 rounded-2xl p-5 shadow-2xs space-y-4">
                        <div className="flex items-center gap-2 border-b border-slate-100 pb-3">
                          ...
                          <div>
                            ...
                            <p className="text-[11px] text-slate-500 font-medium">
                              Outpatient management instructions & lifestyle
                              advice
                            </p>
                          </div>
                        </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-xs">
                          <div className="space-y-1.5">
                            ...
                            <textarea
                              ...
                              onChange={(e) =>
                                setTreatmentRecommendations(e.target.value)
                              }
                          ...
                          <div className="space-y-1.5">
                            <label className="font-extrabold text-slate-800 block text-xs">
                              Follow-Up & Outpatient Instructions
                            </label>
                            <textarea
                              rows={4}
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <h3 className="font-extrabold text-slate-900 text-base">
                                Consultation Summary & Final Review
                              </h3>
                              <p className="text-xs text-slate-500 font-medium">
                                Verify all recorded details before final sign &
                                save to permanent EHR
                              </p>
                            </div>
                          </div>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                          ...
                          <div className="bg-slate-50/80 p-4.5 rounded-2xl border border-slate-200/80 space-y-3">
                            ...
                            <div className="grid grid-cols-2 gap-2 text-xs font-semibold text-slate-800">
                              <div className="bg-white p-2.5 rounded-xl border border-slate-200/60">
                                BP: <strong>{vitalBP}</strong> mmHg
                              </div>
                              <div className="bg-white p-2.5 rounded-xl border border-slate-200/60">
                                HR: <strong>{vitalHR}</strong> bpm
                              </div>
                              <div className="bg-white p-2.5 rounded-xl border border-slate-200/60">
                                Temp: <strong>{vitalTemp}</strong> °C
                              </div>
                              <div className="bg-white p-2.5 rounded-xl border border-slate-200/60">
                                Resp: <strong>{vitalResp}</strong> bpm
                              </div>
                            </div>

                            <div className="pt-2 border-t border-slate-200/60 space-y-1">
                              <strong className="text-slate-800 block text-[11px] uppercase">
                          ...
                          <div className="bg-slate-50/80 p-4.5 rounded-2xl border border-slate-200/80 space-y-3">
                            <h4 className="font-extrabold text-slate-900 flex items-center gap-2 text-xs uppercase tracking-wider text-emerald-900">
                              ...
                              Diagnoses & Orders Summary
                            </h4>

                            <div>
                              <strong className="text-slate-800 block text-[11px] uppercase mb-1">
                            ...
                            <div className="pt-2 border-t border-slate-200/60">
                              ...
                              <div className="space-y-1">
                                {rxList.map((rx, i) => (
                                  <div
                                    key={i}
                                    className="bg-white p-2 rounded-xl border border-slate-200/60 flex justify-between items-center text-xs"
                                  >
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  ...
                  <div className="bg-amber-50 border border-amber-200 p-4 rounded-2xl flex items-start gap-3 text-xs text-amber-900">
                    ...
                    <div>
                      <h4 className="font-extrabold text-amber-950">
                        Patient Drug Safety & Allergy Check
                      </h4>
                      <p className="text-amber-800 font-medium mt-0.5">
                        ...
                        . Ensure prescribed pharmaceuticals do not contain
                        penicillin derivatives or beta-lactam cross-reactivity.
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="bg-white border border-slate-200/90 p-5 rounded-2xl space-y-4 text-xs shadow-2xs">
                    <label className="font-extrabold text-slate-900 block text-sm flex items-center gap-2">
                      <Pill className="w-4 h-4 text-emerald-600" /> Prescribe
                      New Medication
                    </label>
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          ...
                          },
                          {
                            ...
                            freq: "Once Daily",
                            dur: "14 Days",
                            inst: "Take 30 mins before breakfast",
                          },
                          {
                            name: "Paracetamol",
                            dose: "1000 mg",
                  ...
                  <div className="space-y-2 text-xs">
                    <h4 className="font-extrabold text-slate-900">
                      Active Session Prescriptions ({rxList.length})
                    </h4>
                    <div className="space-y-2">
                      {rxList.map((rx, idx) => (
                        <div
                          key={idx}
                          className="bg-emerald-50/80 border border-emerald-200 p-3.5 rounded-2xl flex justify-between items-center text-xs"
                        >
                          <div className="space-y-0.5">
                            ...
                            <p className="text-slate-700 font-medium">
                              ...
                              • Frequency: {rx.freq} • Duration: {rx.duration}
                            </p>
                ...
                <div className="space-y-6 animate-in fade-in duration-200">
                  <div className="bg-white border border-slate-200/90 p-5 rounded-2xl space-y-4 text-xs shadow-2xs">
                    <label className="font-extrabold text-purple-950 block text-sm flex items-center gap-2">
                      ...
                      Requisition Diagnostic Lab / Radiology Test
                    </label>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                      ...
                      <div>
                        ...
                        <select
                          ...
                          onChange={(e) =>
                            setNewLabPriority(e.target.value as any)
                          }
                          className="w-full p-2.5 border border-slate-200 rounded-xl bg-slate-50 focus:bg-white font-bold text-slate-800"
                        >
                    ...
                    <div>
                      <label className="font-bold text-slate-700 block mb-1">
                        Clinical Justification & Technician Instructions
                      </label>
                      <textarea
                        rows={2}
                        value={newLabNotes}
                    ...
                    <div className="pt-1">
                      ...
                      <div className="flex flex-wrap gap-1.5">
                        {[
                          ...
                          {
                            name: "Renal Function Test (E/U/Cr)",
                            cat: "Laboratory",
                          },
                          {
              ...
              <div className="bg-white p-4 rounded-2xl border border-slate-200/90 shadow-2xs flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="flex items-center gap-2">
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">
                    Signing commits consultation notes, prescriptions, and lab
                    requisitions to the national EHR database.
                  </p>
                </div>
                <button
                  type="button"
                ...
                <div className="flex flex-wrap gap-3 items-center justify-between">
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-xs">
                    ...
                    >
                      ...
                      <option value="General Hospital Abuja">
                        General Hospital Abuja
                      </option>
                      <option value="National Diagnostic Lab">
                        National Diagnostic Lab
                      </option>
                    </select>
                ...
                {filteredTimeline.length === 0 ? (
                  <div className="bg-white border border-slate-200 rounded-2xl p-12 text-center space-y-2">
                    ...
                    <p className="font-bold text-slate-700 text-sm">
                      No medical visits match your filter parameters.
                    </p>
                    <p className="text-xs text-slate-400">
                      Try adjusting your keyword search or hospital filter.
                    </p>
                  </div>
                ) : (
                    ...
                    >
                      ...
                      <div className="bg-blue-50/50 border border-blue-100 p-4 rounded-xl space-y-1.5 text-xs">
                        <span className="font-bold text-blue-900 text-[11px] uppercase block flex items-center gap-1">
                          <FileText className="w-3.5 h-3.5 text-blue-600" />{" "}
                          Physician Clinical Findings & Observations
                        </span>
                          ...
                          {visit.prescriptions.length > 0 ? (
                            <div className="space-y-1.5 text-[11px]">
                              {visit.prescriptions.map((rx) => (
                                <div
                                  key={rx.id}
                                  className="bg-emerald-50 text-emerald-950 p-2 rounded-lg font-medium border border-emerald-100 flex justify-between items-center"
                                >
                                  <div>
                          ...
                          {visit.labRequests.length > 0 ? (
                            <div className="space-y-1.5 text-[11px]">
                              {visit.labRequests.map((lab) => (
                                <div
                                  key={lab.id}
                                  className="bg-purple-50 text-purple-950 p-2 rounded-lg font-medium border border-purple-100"
                                >
                                  <div className="flex justify-between items-center">
                                    <span className="font-bold">
                                      {lab.testType}
                      ...
                      {visit.followUpNotes && (
                        <div className="text-[11px] text-slate-500 font-semibold pt-1 border-t border-slate-100 flex justify-between items-center">
                          <span className="flex items-center gap-1.5">
                            ...
                            Follow-up Plan: {visit.followUpNotes}
                          </span>
                          <span className="text-[10px] text-slate-400">
                            Recorded by {visit.doctorName}
                          </span>
                        </div>
                      )}
                    </div>
            ...
            <div className="space-y-5">
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs space-y-3">
                <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                  <Activity className="w-4 h-4 text-blue-600" /> Current
                  Baseline Vitals
                </h3>
              ...
              <div className="bg-white border border-slate-200 rounded-2xl p-5 shadow-2xs space-y-3">
                <h3 className="font-extrabold text-slate-900 text-sm flex items-center gap-2">
                  <Pill className="w-4 h-4 text-emerald-600" /> Active
          ...
          {activeTab === "notes" && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <div>
                  <h3 className="font-extrabold text-slate-900 text-base">
                    Signed Permanent Clinical Notes
                  </h3>
                  <p className="text-xs text-slate-500">
                    Read-only archived consultation records preserved for
                    medical audit and care continuity.
                  </p>
                </div>
              </div>
              ...
              <div className="space-y-4">
                {timelineItems.map((note) => (
                  <div
                    key={note.id}
                    className="bg-slate-50 border border-slate-200 p-4 rounded-xl text-xs space-y-2"
                  >
          ...
          {activeTab === "labs" && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-4">
              <div className="flex justify-between items-center border-b border-slate-100 pb-3">
                <h3 className="font-extrabold text-slate-900 text-base">
                  Diagnostic Laboratory Requests & Results
                </h3>
                <button
                  onClick={() => {
                    setActiveTab("consultation");
                  ...
                  >
                    ...
                    {lab.resultSummary ? (
                      ...
                      <p className="text-slate-400 italic">
                        Test pending laboratory processing.
                      </p>
                    )}
                  </div>
                ))}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                {records.map((rec) => (
                  <div
                    ...
                    className="bg-slate-50 border border-slate-200 p-4 rounded-xl space-y-2"
                  >
                    <span className="font-mono text-[10px] text-blue-600 font-bold uppercase block">
                      {rec.fileType} • {rec.specialty}
                    </span>
          ...
          {activeTab === "access" && (
            <div className="bg-white border border-slate-200 rounded-2xl p-6 shadow-2xs space-y-5 text-xs">
              <div>
                <h3 className="font-extrabold text-slate-900 text-base">
                  Active Hospital Authorizations & Audit Logs
                </h3>
                <p className="text-xs text-slate-500">
                  Hospitals and clinicians currently granted consent to inspect
                  or edit this EHR record.
                </p>
              </div>
                ...
                {hospitalAccesses.map((acc: any) => (
                  ...
                  >
                    <div className="space-y-1">
                      ...
                      <p className="text-[11px] text-slate-500">
                        Granted: {acc.grantedDate} • Authorized Depts:{" "}
                        {acc.authorizedDepartments.join(", ")}
                      </p>
                    </div>
              ...
              <div className="border-t border-slate-100 pt-4 space-y-2">
                <h4 className="font-bold text-slate-800 text-xs">
                  Recent Medical Access Audit Trail
                </h4>
                <div className="divide-y divide-slate-100 font-mono text-[11px]">
                  {auditLogs.map((log) => (
                    ...
                    >
                      <span>
                        {log.timestamp}: {log.actorName} ({log.actorRole}) -{" "}
                        {log.action}
                      </span>
      ...
      <button
        ...
        title="Open AI Decision Support"
      >
          ...
          <div className="bg-white rounded-3xl max-w-2xl w-full shadow-2xl border border-indigo-100 overflow-hidden flex flex-col max-h-[85vh] animate-in zoom-in-95 duration-200">
            ...
            <div className="p-5 border-b border-slate-100 flex items-center justify-between bg-gradient-to-r from-indigo-50 via-blue-50 to-white">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-medium">
                    Gemma 4 Medical Engine • Diagnostic reasoning, drug
                    interaction screening & clinical support
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-5 overflow-y-auto space-y-4 text-xs flex-1">
              ...
              <div className="bg-indigo-50/70 border border-indigo-100 p-3.5 rounded-2xl flex items-center justify-between gap-3 text-xs">
                <div className="space-y-0.5">
                  ...
                  <p className="text-[11px] text-indigo-800 font-medium">
                    Vitals: BP {vitalBP} mmHg • HR {vitalHR} bpm • Allergies:
                    Penicillin, Dust Mites
                  </p>
                </div>
              ...
              <div className="space-y-2 pt-1">
                <label className="font-bold text-slate-800 text-xs block">
                  Clinical Query or Protocol Search
                </label>
                <div className="flex gap-2">
                  <input
                    type="text"
              ...
              {aiResponse && (
                <div className="bg-slate-50 p-4 rounded-2xl border border-indigo-200 text-xs text-slate-800 space-y-3 animate-in fade-in duration-200">
                  <div className="flex items-center justify-between border-b border-indigo-100 pb-2">
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        alert(
                          "AI recommendation appended to clinical progress notes!",
                        );
                      }}
                  ...
                  <p className="whitespace-pre-wrap leading-relaxed text-slate-700 font-sans">
                    {aiResponse}
                  </p>
                </div>
              )}
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex justify-between items-center text-xs">
              <span className="text-[11px] text-slate-400 font-medium">
                Verified with Federal Ministry of Health Clinical Knowledge
                Protocols
              </span>
              <button
                type="button"
